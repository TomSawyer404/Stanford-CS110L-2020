...
impl Process {
    ...
    pub fn list_fds(&self) -> Option<Vec<usize>> {
        ...
        for entry in fs::read_dir(path).ok()? {
            let entry = entry.ok()?;
            let path = entry.path();
            let file_name = path.file_name()?; // Type of file_name is OsStr
            let fd = file_name.to_str()?.parse::<usize>().ok()?;
            ans.push(fd);
        }
        Some( ans )
    }
    ...
    pub fn print(&self) {
        ...
        match self.list_open_files() {
            None => println!(
                "Waring: could not inspect file descriptors for this process!\
                 Itmight have exited just as we were about to look at its fd table, \
