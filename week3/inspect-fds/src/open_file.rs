...
impl OpenFile {
    ...
    pub fn from_fd(pid: usize, fd: usize) -> Option<OpenFile> {
        ...
        let fd_points2 = fs::read_link( &path_str ).ok()?;
        let fname: String = OpenFile::path_to_name( fd_points2.to_str()? );

        let path_str = format!("/proc/{}/fdinfo/{}", pid, fd);
        let fd_content: String = fs::read_to_string(path_str).ok()?;
        let cursor = OpenFile::parse_cursor( &fd_content[..] )?;
        let mode = OpenFile::parse_access_mode( &fd_content[..] )?;

        let ret_obj = OpenFile::new(fname, cursor, mode);
        Some( ret_obj )
    }
